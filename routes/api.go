...
func RouterInit() {
	...
	admin.Get("userbyid", AC.UserByID)
	admin.Get("/userSearch", AC.UserSearch)
	admin.Post("/user", AC.AddUser)
	admin.Post("/user/verify", AC.UserVerification)
	admin.Delete("DeleteUser", AC.DeleteUser)
	admin.Put("/laws", AC.UpdateLaw)
	admin.Delete("/laws", AC.DeleteLaw)
	// ! laws route
